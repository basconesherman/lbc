<?php
	...
	$result2 = mysql_query("SELECT * FROM tblusers WHERE email='$email' AND password='$password'");
	while($row2 = mysql_fetch_array($result2)){
		$check_pos2 = $row2['type'];
	}
	...
	if ($check_pos=='admin'){
		...
		if($result) {
			if(mysql_num_rows($result) > 0) {
				...
				setcookie("mail", $_SESSION['SESS_MEMBER_ID'], time()+14400);
				session_write_close();
				//if ($level="admin"){
	...
	if ($check_pos=='client'){
		...
		if($result) {
			if(mysql_num_rows($result) > 0) {
				...
				$member = mysql_fetch_assoc($result);
				$_SESSION['SESS_MEMBER_ID'] = $member['email'];
				setcookie("mail", $_SESSION['SESS_MEMBER_ID'], time()+7200);
				session_write_close();
				//if ($level="admin"){
